...
cdef class Compute_Similarity_Cython:
    ...
    cdef applyPearsonCorrelation(self, dataMatrix):
        ...
        cdef int64_t colIndex, innerIndex, start_pos, end_pos
    ...
    cdef applyAdjustedCosine(self, dataMatrix):
        ...
        cdef int64_t rowIndex, innerIndex, start_pos, end_pos
    ...
    #     return pointer
    #
    # cdef data_pointer_s getItemsRatedByUser_pointer(self, int64_t user_id):
    #
    #
    #     cdef data_pointer_s pointer = data_pointer_s()
    #
    ...
    cdef computeItemSimilarities(self, int64_t item_id_input):
        """
        ...
        That is VERY slow due to the common user part, in which a int64_t data structure is looped multiple times.
        ...
        cdef int64_t user_index, user_id, item_index, item_id, item_id_second
    ...
    def compute_similarity(self, start_col=None, end_col=None):
        ...
        cdef int64_t topKItemIndex

        cdef int64_t[:] top_k_idx
        ...
        cdef int64_t sparse_data_pointer = 0
...
cdef int64_t[:] argsort(double[:] this_item_weights, int TopK):

    #start_time = time.time()
    cdef array[int64_t] template_zero = array('l')
    cdef array[int64_t] result = clone(template_zero, TopK, zero=False)
