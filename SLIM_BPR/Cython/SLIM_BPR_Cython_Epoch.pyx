...
cdef struct BPR_sample:
    ...
    int64_t pos_item
    int64_t neg_item
    int64_t seen_items_start_pos
    int64_t seen_items_end_pos
...
cdef class SLIM_BPR_Cython_Epoch:
    ...
    def epochIteration_Cython(self):
        ...
        cdef int64_t totalNumberOfBatch = int(self.numPositiveIteractions / self.batch_size) + 1

        cdef int64_t start_time_epoch = time.time()
        cdef int64_t start_time_batch = time.time()

        cdef BPR_sample sample
        cdef int64_t i, j
        cdef int64_t index, seenItem, numCurrentBatch, itemId
...
cdef matrix_element_tree_s * pointer_new_matrix_element_tree_s(int64_t column, double data, matrix_element_tree_s *higher,  matrix_element_tree_s *lower):
...
cdef class Sparse_Matrix_Tree_CSR:
    ...
    cpdef double add_value(self, int64_t row, int64_t col, double value):
        """
    ...
    cpdef double get_value(self, int64_t row, int64_t col):
        """
    ...
    cpdef get_scipy_csr(self, int64_t TopK = False):
        """
    ...
    cpdef rebalance_tree(self, int64_t TopK = False):
        """
    ...
    cdef matrix_element_tree_s* build_tree_from_list_flat(self, matrix_element_tree_s* flat_list_head):
        ...
        cdef int64_t list_length = 0
        cdef int64_t middle_element_step = 0
    ...
    cdef matrix_element_tree_s* topK_selection_from_list(self, matrix_element_tree_s* head, int64_t TopK):
        """
        ...
        cdef matrix_element_tree_s *current_element
        cdef int64_t list_length, index, selected_count
...
##################################################################################################################
    ...
    cpdef test_list_tee_conversion(self, int64_t row):
        """
    ...
    cpdef test_topK_from_list_selection(self, int64_t row, int64_t topK):
        """
        ...
        assert len(column_topK) <= topK,\
            "TopK extracted list is int64_ter than desired value. Desired is {}, while list is {}".format(topK, len(column_topK))
...
cdef class Triangular_Matrix:
    ...
    def __init__(self, int64_t num_rows, int isSymmetric = False):
    ...
    cpdef get_scipy_csr(self, int64_t TopK = False):
        ...
        cdef np.ndarray[int64_t, ndim=1] top_k_partition, top_k_partition_sorting
